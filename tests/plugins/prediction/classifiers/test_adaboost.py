...
from autoprognosis.plugins.prediction.classifiers.plugin_adaboost import plugin
...
def from_api() -> PredictionPlugin:
    return Predictions().get("adaboost")
...
def test_adaboost_plugin_sanity(test_plugin: PredictionPlugin) -> None:
...
def test_adaboost_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "adaboost"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_adaboost_plugin_type(test_plugin: PredictionPlugin) -> None:
...
def test_adaboost_plugin_hyperparams(test_plugin: PredictionPlugin) -> None:
    ...
    assert test_plugin.hyperparameter_space()[2].name == "base_estimator"
...
def test_adaboost_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
