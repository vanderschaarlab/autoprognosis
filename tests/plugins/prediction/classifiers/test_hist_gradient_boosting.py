...
from autoprognosis.plugins.prediction.classifiers.plugin_hist_gradient_boosting import (
    plugin,
)
...
def from_api() -> PredictionPlugin:
    return Predictions().get("hist_gradient_boosting")
...
def test_hist_gradient_boosting_plugin_sanity(test_plugin: PredictionPlugin) -> None:
...
def test_hist_gradient_boosting_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "hist_gradient_boosting"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_hist_gradient_boosting_plugin_type(test_plugin: PredictionPlugin) -> None:
...
def test_hist_gradient_boosting_plugin_hyperparams(
    test_plugin: PredictionPlugin,
) -> None:
    assert len(test_plugin.hyperparameter_space()) == 2
    assert test_plugin.hyperparameter_space()[0].name == "max_depth"
    assert test_plugin.hyperparameter_space()[1].name == "learning_rate"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_hist_gradient_boosting_plugin_fit_predict(
    test_plugin: PredictionPlugin,
) -> None:
...
def test_param_search() -> None:
    ...
    study.optimize(objective, n_trials=2, timeout=60)
