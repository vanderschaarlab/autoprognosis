...
from autoprognosis.plugins.prediction.classifiers.plugin_decision_trees import plugin
from autoprognosis.utils.serialization import load_model, save_model
from autoprognosis.utils.tester import evaluate_estimator


def from_api() -> PredictionPlugin:
    return Predictions().get("decision_trees")
...
def test_decision_trees_plugin_sanity(test_plugin: PredictionPlugin) -> None:
...
def test_decision_trees_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "decision_trees"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_decision_trees_plugin_type(test_plugin: PredictionPlugin) -> None:
...
def test_decision_trees_plugin_hyperparams(test_plugin: PredictionPlugin) -> None:
    assert len(test_plugin.hyperparameter_space()) == 1
    assert test_plugin.hyperparameter_space()[0].name == "criterion"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_decision_trees_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
