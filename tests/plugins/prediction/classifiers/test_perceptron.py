...
from autoprognosis.plugins.prediction.classifiers.plugin_perceptron import plugin
from autoprognosis.utils.serialization import load_model, save_model
from autoprognosis.utils.tester import evaluate_estimator


def from_api() -> PredictionPlugin:
    return Predictions().get("perceptron")
...
def test_perceptron_plugin_sanity(test_plugin: PredictionPlugin) -> None:
...
def test_perceptron_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "perceptron"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_perceptron_plugin_type(test_plugin: PredictionPlugin) -> None:
...
def test_perceptron_plugin_hyperparams(test_plugin: PredictionPlugin) -> None:
    assert len(test_plugin.hyperparameter_space()) == 2
    assert test_plugin.hyperparameter_space()[0].name == "alpha"
    assert test_plugin.hyperparameter_space()[1].name == "penalty"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_perceptron_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
    X, y = load_iris(return_X_y=True)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)

    y_pred = test_plugin.fit(X_train, y_train).predict(X_test).to_numpy()

    assert np.abs(np.subtract(y_pred, y_test)).mean() < 1.5
