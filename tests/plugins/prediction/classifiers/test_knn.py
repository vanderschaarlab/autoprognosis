...
from autoprognosis.plugins.prediction.classifiers.plugin_knn import plugin
from autoprognosis.utils.serialization import load_model, save_model
from autoprognosis.utils.tester import evaluate_estimator


def from_api() -> PredictionPlugin:
    return Predictions().get("knn")
...
def test_knn_plugin_sanity(test_plugin: PredictionPlugin) -> None:
...
def test_knn_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "knn"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_knn_plugin_type(test_plugin: PredictionPlugin) -> None:
...
def test_knn_plugin_hyperparams(test_plugin: PredictionPlugin) -> None:
    assert len(test_plugin.hyperparameter_space()) == 4
    assert test_plugin.hyperparameter_space()[0].name == "p"
    assert test_plugin.hyperparameter_space()[1].name == "algorithm"
    assert test_plugin.hyperparameter_space()[2].name == "weights"
    assert test_plugin.hyperparameter_space()[3].name == "n_neighbors"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_knn_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
