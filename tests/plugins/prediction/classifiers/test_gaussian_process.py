...
from autoprognosis.plugins.prediction.classifiers.plugin_gaussian_process import plugin
from autoprognosis.utils.serialization import load_model, save_model
from autoprognosis.utils.tester import evaluate_estimator


def from_api() -> PredictionPlugin:
    return Predictions().get("gaussian_process")
...
def test_gaussian_process_plugin_sanity(test_plugin: PredictionPlugin) -> None:
...
def test_gaussian_process_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "gaussian_process"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_gaussian_process_plugin_type(test_plugin: PredictionPlugin) -> None:
...
def test_gaussian_process_plugin_hyperparams(test_plugin: PredictionPlugin) -> None:
...
def test_gaussian_process_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
