...
from autoprognosis.plugins.prediction.classifiers.plugin_extra_tree_classifier import (
    plugin,
)
...
def from_api() -> PredictionPlugin:
    return Predictions().get("extra_tree_classifier")
...
def test_extra_tree_classifier_plugin_sanity(test_plugin: PredictionPlugin) -> None:
...
def test_extra_tree_classifier_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "extra_tree_classifier"


@pytest.mark.parametrize(
    "test_plugin", [from_api(), from_module(), from_serde(), from_pickle()]
)
def test_extra_tree_classifier_plugin_type(test_plugin: PredictionPlugin) -> None:
...
def test_extra_tree_classifier_plugin_hyperparams(
...
def test_extra_tree_classifier_plugin_fit_predict(
    test_plugin: PredictionPlugin,
) -> None:
