...
from autoprognosis.plugins.prediction.risk_estimation.plugin_lognormal_aft import plugin
from autoprognosis.utils.metrics import evaluate_brier_score, evaluate_c_index


def from_api() -> PredictionPlugin:
    return Predictions(category="risk_estimation").get("lognormal_aft")
...
def test_lognormal_aft_plugin_sanity(test_plugin: PredictionPlugin) -> None:
    assert test_plugin is not None


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_serde()])
def test_lognormal_aft_plugin_name(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.name() == "lognormal_aft"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_serde()])
def test_lognormal_aft_plugin_type(test_plugin: PredictionPlugin) -> None:
    assert test_plugin.type() == "prediction"
    assert test_plugin.subtype() == "risk_estimation"


@pytest.mark.parametrize("test_plugin", [from_api(), from_module(), from_serde()])
def test_lognormal_aft_plugin_hyperparams(test_plugin: PredictionPlugin) -> None:
...
def test_lognormal_aft_plugin_fit_predict(test_plugin: PredictionPlugin) -> None:
    rossi = load_rossi()
